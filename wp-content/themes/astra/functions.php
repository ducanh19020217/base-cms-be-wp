...
/**
 * Expose WooCommerce fields to REST API
 */
add_action( 'rest_api_init', function () {
    ...
    register_rest_field( 'product', 'regular_price', array(
        'get_callback' => function( $object ) {
            $product = wc_get_product( $object['id'] );
            return $product ? $product->get_regular_price() : 0;
        }
    ));
    ...
    register_rest_field( 'product', 'sale_price', array(
        'get_callback' => function( $object ) {
            $product = wc_get_product( $object['id'] );
            return $product ? $product->get_sale_price() : 0;
        }
    ));

    // Expose Featured Image URL
    register_rest_field( 'product', 'featured_image_url', array(
        'get_callback' => function( $object ) {
            $product = wc_get_product( $object['id'] );
    ...
    // Expose Attributes
    register_rest_field( 'product', 'attributes_data', array(
        'get_callback' => function( $object ) {
            $product = wc_get_product( $object['id'] );
    ...
    // Expose Tags Data
    register_rest_field( 'product', 'tags_data', array(
        'get_callback' => function( $object ) {
            $product = wc_get_product( $object['id'] );
            if ( ! $product ) return array();
            
            $tags = wc_get_product_tag_list( $product->get_id(), ', ', '', '' );
            // Parse the HTML links returned by wc_get_product_tag_list if needed, 
            // or just get terms directly for cleaner data
